      ...
      <Fragment>
        ...
        <main className="flex column w-700 txt-al-c">
          ...
          <form onSubmit={ e => this.getProbability(e) } className="flex column m-t-20">
            ...
            <select id="faculty" autoFocus onChange={ e => this.handleFacultySelect(e) } className="w-100 p-5 m-t-10 m-b-20 bd-0 ol-0 bd-r-20 bx-sh bc-c1 main-text fs-in ttn-3 pointer">
              {
            ...
            <select id="course" onChange={ e => this.setState({ selectedCourse: e.target.value }) } className="w-100 p-5 m-t-10 m-b-20 bd-0 ol-0 bd-r-20 bx-sh bc-c1 main-text fs-in ttn-3 pointer">
              {
            ...
            <input type="number" min="0" max="100" step="0.01" id="aggregate" onChange={ e => this.setState({ aggregate: e.target.value }) } className="w-100 p-5 m-t-10 m-b-20 bd-0 ol-0 bd-r-20 bx-sh bc-c1 main-text fs-in txt-al-c ttn-3" required />
            <input type="submit" value="Check" className="w-100 p-5 m-t-10 m-b-20 bd-0 ol-0 bd-r-20 bx-sh bc-c1 main-text fs-in ttn-3 pointer" />
          </form>
        </main>
      </Fragment>
    );
